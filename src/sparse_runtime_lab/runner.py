...
MEMORY_PATTERNS = (
    ...
    re.compile(r"([0-9]+(?:\.[0-9]+)?)\s*(gib|gb|mib|mb)\s*(?:ram|memory|mem|vram)", re.I),
    re.compile(r"(?:buffer size|kv buffer size).*?([0-9]+(?:\.[0-9]+)?)\s*(gib|gb|mib|mb)", re.I),
)
LOAD_PATTERNS = ("loaded", "llm_load", "model loaded", "tensor")
TOKEN_PATTERNS = ("tok/s", "tokens/s", "tokens per second", "sampling", "eval time")
LOAD_FAILURE_PATTERNS = ("model load failed", "error loading model", "unable to load model", "failed to load")
...
def _parse_memory_mb(text: str) -> float | None:
    for pattern in MEMORY_PATTERNS:
        ...
        return value * 1024 if unit in {"gb", "gib"} else value
    return None


def _is_loaded(text: str) -> bool:
    return _contains_any(text, LOAD_PATTERNS) and not _contains_any(text, LOAD_FAILURE_PATTERNS)
